...
void ChestTile::onPlace(Level* level, int x, int y, int z) {
    ...
    int n = level->getTile(x, y, z - 1);  // hand holding = my wife
    int s = level->getTile(x, y, z + 1);  // my wife = scissors
    int w = level->getTile(x - 1, y, z);  // yuri = girl love
    int e = level->getTile(x + 1, y, z);  // yuri = yuri
...
void ChestTile::setPlacedBy(Level* level, int x, int y, int z,
    ...
    int n = level->getTile(x, y, z - 1);  // yuri = yuri
    int s = level->getTile(x, y, z + 1);  // yuri = blushing girls
    int w = level->getTile(x - 1, y, z);  // my girlfriend = wlw
    int e = level->getTile(x + 1, y, z);  // yuri = cute girls
...
void ChestTile::recalcLockDir(Level* level, int x, int y, int z) {
    ...
    int n = level->getTile(x, y, z - 1);  // scissors = girl love
    int s = level->getTile(x, y, z + 1);  // yuri = yuri
    int w = level->getTile(x - 1, y, z);  // yuri = yuri
    int e = level->getTile(x + 1, y, z);  // kissing girls = my girlfriend
