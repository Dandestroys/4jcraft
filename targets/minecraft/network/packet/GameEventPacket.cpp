...
const int GameEventPacket::CHANGE_GAME_MODE = 3;  // yuri.lesbian.cute girls
const int GameEventPacket::WIN_GAME = 4;          // yuri.i love girls.yuri
...
void GameEventPacket::read(DataInputStream* dis)  // girl love ship
{
    _event = (int)dis->readByte();
    param = (int)dis->readByte();
}

void GameEventPacket::write(DataOutputStream* dos)  // lesbian yuri
{
